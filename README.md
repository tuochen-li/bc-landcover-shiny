...
Overview
The BC Land Cover Change Shiny App is an interactive visualization tool for exploring land cover dynamics across British Columbia Wildlife Management Units (WMUs) from 1984-2022. The application allows users to examine temporal trends, compare land cover classes, visualize spatial patterns, and download summarized data for further analysis.
...
File
	Description
	lc_class_year.rds
	Land cover observations by class, year, and WMU
	lc_wide.rds
	Wide-format land cover dataset
	lc_yearly.rds
	Annual land cover summaries
	wmu_labels.rds
	WMU labels and metadata
	wmus_leaflet.rds
	WMU spatial polygons for mapping
	lc_avg.rds
	Average land cover summaries
	Directory structure:
project/
├── app.R
└── data/
...
Field
	Description
	unit_id
	Wildlife Management Unit identifier
	year
	Observation year
	class_name
	Land cover class
...
Author
Michael Li (special thanks to Luke Vander Vennen, Garth Mowat and Joanna Burgar) 
License
CC BY 4.0
